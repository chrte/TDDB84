...
public class Door extends MapSite {
	...
	public void enter(MovingObject p) {
		if (isOpen) {
			otherSideFrom(Stable.instance().getRoom(p.currentX, p.currentY)).enter(p);
		}
	}
	...
	public void paint(Graphics g, Room r) {
		if (r.getOffsetY() < otherSideFrom(r).getOffsetY()) {
			//Down
			g.drawImage(Images.instance().getDoorHorizontalImage(Room.getSize() - 2*wallSize, 2*halfDoorSize, isOpen),
		...
		if (r.getOffsetX() < otherSideFrom(r).getOffsetX()) {
			//Left
			g.drawImage(Images.instance().getDoorVerticalImage(2*halfDoorSize, Room.getSize() - 2*wallSize, isOpen),
