...
public class BoxDoor extends Door{
	...
	public void paint(Graphics g, Room r) {
		if (r.getOffsetY() < otherSideFrom(r).getOffsetY()) {
			//Down
			g.drawImage(Images.instance().getBoxDoorHorizontalImage(Room.getSize() - 2*wallSize, 2*halfDoorSize, isOpen),
		...
		if (r.getOffsetX() < otherSideFrom(r).getOffsetX()) {
			//Left
			g.drawImage(Images.instance().getBoxDoorVerticalImage(2*halfDoorSize, Room.getSize() - 2*wallSize, isOpen),
